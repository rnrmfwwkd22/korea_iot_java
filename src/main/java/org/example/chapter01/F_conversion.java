...
public class F_conversion {
    //메인메서드 : 파일 단위의 실행을 담당하는 도구
//    - main  or  psvm 단축기
    public static void main(String[] args) {
        /* 자바의 형 변환(Type Conversion)
         :하나의 차입을 다른 타입으로 바꾸는 것

         표현가능한 범위 기준에 따른 변환 규칙
         : 메모리 크기와 순서가 다를 수 있다
        ...
        bigBox = smallBox; // 재할당 - 기존으 ㅣ변수에 변수에 새로운 값을 할당(삽입)

//       cf) 변수는 재할당 시 기존의 값이 새로운 값을 '변환'
...
//         : 큰 자료형을 작은 자료형으로 변환( 오른쪽에서 왼쪽)
        ...
        int intNumber = 1234;
        float floatNumbber = intNumber;

        //강제 형 변환
        //작은타입 변수명 = (작은타입) 큰데이터값;
        int NUmber2 = (int) floatNumbber;

        long aa = 32L; //long타입은 데이터 뒤에 알파벳 L,l필수
        int bb =(int) aa;
        short cc = (short) bb;
        byte dd = (byte) cc;
